...
const [auth, _, db] = await setupFirebase(
  {
  ...
  if (filename.split(".").pop() === "dat") {
    ...
    try {
      ...
      if (textDataArray.length === 2) {
        ...
        batch.set(
          doc(db, "companies", process.env.CID, noSpaceFilename, "header"),
          {
            ...
            woodSaturationPointDensity: textDataArrayHeader[14].trim(),
        ...
        for (let i = 0; i < textDataArrayPhases.length / 18 - 1; i++) {
          batch.set(
            ...
            ),
            {
              ...
              phaseRunTime: textDataArrayPhases[i * 18 + 1].trim(),
              phaseEndWoodMoisture: textDataArrayPhases[i * 18 + 2].trim(),
              phaseEndPotential: textDataArrayPhases[i * 18 + 3].trim(),
              phaseEndUGL: textDataArrayPhases[i * 18 + 4].trim(),
              phaseEndTemperature: textDataArrayPhases[i * 18 + 5].trim(),
              phaseEndRPM: textDataArrayPhases[i * 18 + 6].trim(),
              phaseEndPhantom: textDataArrayPhases[i * 18 + 7].trim(),
              phaseHeating: textDataArrayPhases[i * 18 + 8].trim(),
              phaseHatch: textDataArrayPhases[i * 18 + 9].trim(),
              phaseSpray: textDataArrayPhases[i * 18 + 10].trim(),
              phaseOptionalRelais1: textDataArrayPhases[i * 18 + 11].trim(),
              phaseOptionalRelais2: textDataArrayPhases[i * 18 + 12].trim(),
              phaseOptionalRelais3: textDataArrayPhases[i * 18 + 13].trim(),
              phaseID: textDataArrayPhases[i * 18 + 14].trim(),
              phaseHeatingValue: textDataArrayPhases[i * 18 + 15].trim(),
              phaseHatchValue: textDataArrayPhases[i * 18 + 16].trim(),
              phaseSprayValue: textDataArrayPhases[i * 18 + 17].trim(),
            }
          );
        }
        batch.commit();
      } else {
        log.warn(`${filename} file has incorrect format and is ignored`, 1630);
      }
    } catch (e) {
